...
def get_iterator(args, kv, data_shape):
    train_data, val_data = download_cifar10(args.data_dir)
...
def train_img(args, ctx):
    ...
    if (args.load_epoch > 0):
        print "load model from %s epoch %d"%(args.prefix, args.load_epoch)
        mod = mx.module.module.Module.load(args.prefix, args.load_epoch, logger=logger,
                       context=ctx, work_load_list=args.work_load_list)

    epoch_end_callback = mx.callback.do_checkpoint(args.prefix, period=1)
    ...
    mod.fit(train_iter, val_iter, eval_metric=eval_metric, batch_end_callback=batch_end_callback,
            ...
            allow_missing=True,
            begin_epoch=args.load_epoch, num_epoch=args.num_epoch, validation_metric='acc')

    print "Train done for epoch: %s"%args.num_epoch


def extract_feature(args, ctx):
    # load model
    assert(args.load_epoch > 0)
    sym, arg_params, aux_params = mx.model.load_checkpoint(args.prefix, args.load_epoch)
    feat_sym = sym.get_internals()['fc6_output']
    mod = mx.module.module.Module(feat_sym, context=ctx)
...
def parse_args():
    ...
    parser.add_argument('--load_epoch', dest='load_epoch', help='load epoch of pretrain model',
                        default=0, type=int)
